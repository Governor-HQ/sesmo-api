...
// Bucket a data plan by its validity window, parsed from the plan name.
export function durationBucket(name) {
  const n = String(name || "").toLowerCase();
  let d = null;
  let m = n.match(/(\d+)\s*month/); if (m) d = parseInt(m[1], 10) * 30;
  if (d == null) { m = n.match(/(\d+)\s*week/); if (m) d = parseInt(m[1], 10) * 7; }
  if (d == null) { m = n.match(/(\d+)\s*day/) || n.match(/for\s*(\d+)\s*day/); if (m) d = parseInt(m[1], 10); }
  if (d == null) { if (n.includes("annual") || n.includes("365")) d = 365; else if (n.includes("weekly")) d = 7; else if (n.includes("monthly")) d = 30; }
