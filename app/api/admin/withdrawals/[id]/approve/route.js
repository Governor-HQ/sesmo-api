// POST /api/admin/withdrawals/[id]/approve  { note? } — mark paid (after paying the bank).
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    await pool.query(`select approve_withdrawal($1::uuid,$2::uuid,$3::text)`, [params.id, admin.userId, note]);
    return NextResponse.json({ success: true });
  } catch (e) {
    if (String(e.message || "").includes("WITHDRAWAL_NOT_PENDING"))
    ...
    return NextResponse.json({ success: false, error: "Could not approve." }, { status: 500 });
  }
}
